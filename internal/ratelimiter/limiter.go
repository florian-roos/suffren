...
func (l *Limiter) Check(identifier string, resource string, value uint64, rule Rule) Decision {
	consumed, err := l.getTotalCountInSlidingWindow(identifier, resource, value, rule)
...
// Gets the quantity consummed during the sliding window
func (l *Limiter) getTotalCountInSlidingWindow(identifier string, resource string, value uint64, rule Rule) (uint64, error) {
	now := l.clock()
